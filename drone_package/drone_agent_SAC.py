...
class DroneAgent_SAC(Node):
   def __init__(self, drone_id, agent, control, environment, n_episodes = 10, save_every_n = 5, save_path=None, training=True):
      ...
      self.loss_memory = LossMemory(is_SAC=True)
   ...
   def control_timer_callback(self):
      if not self.environment.done:
         self.trained = False
         if not self.training or self.episode_count == self.trigger:
            self.training = False
         ...
         if self.training:
            self.loss_memory.add_sac_losses(self.agent.learn(verbose=0))
         self.reward_storage.add_reward(reward)                         # Save reward
      elif not self.training: # If this is the testing session
         self.position_memory.save_position_chart(self.save_path+"SAC_positions.png", chart_type="2d")
         self.position_memory.save_position_values(self.save_path+"SAC_positions.txt")
         self.reward_storage.save_rewards_list(self.save_path+"SAC_rewards_list.txt")
   ...
   def save_data(self, final=False):
      if self.save_path != None:
         self.reward_storage.save_reward_chart_mean(self.save_path+"SAC_rewards_mean.jpg", reward_gap=50)
         self.reward_storage.save_reward_chart_median(self.save_path+"SAC_rewards_median.jpg", reward_gap=50)
         self.reward_storage.save_rewards_list(self.save_path+"SAC_rewards_list.txt")
         self.loss_memory.save_sac_losses_chart(self.save_path+"SAC_losses.jpg", color=["red", "blue", "orange", "yellow"], gap=1000)
         self.loss_memory.save_sac_loss_lists(self.save_path+"SAC_losses_list.txt")
         self.agent.save_models(self.save_path, final)
         print("Data saved!")
      return
   ...
   def decode_action(self, action, verbose=0):
      ...
      if verbose > -1:
         ...
         cmd_message += " [" + str(self.x )+ ", " + str(self.y) + "]"
         print(cmd_message)
...
def main(args=None):
   ...
   agent = Agent(alpha=0.001, beta=0.001, temperature=0.025, input_dims=(84,84,1), max_action=1.0, gamma=0.99, n_actions=2, mem_size=1000, tau=0.001,
                 layer1_size=256, layer2_size=256, batch_size=32, reward_scale=1, replace_target=10)
   #agent.load_models("/home/kristijan.segulja/Desktop/Results/SAC/")
   control = Control(yaw=10.0, roll=10.0, throttle=10.0)
   environment = Environment(state_shape=(84,84,1))

   rclpy.init(args=args)
   drone_agent = DroneAgent_SAC(drone_id=d_id,
                                agent=agent,
                                control=control,
                                environment=environment, 
                                save_every_n=5,
                                n_episodes=250,
                                save_path="/home/kristijan.segulja/Desktop/Results/SAC/",
                                training=True)
